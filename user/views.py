...
def login(request):
    ...
    if request.method == "POST":
        ...
        if username and password:  # 确保用户名和密码都不为空
            username = username.strip() # strip() 方法用于移除字符串头尾指定的字符（默认为空格或换行符）或字符序列
            # 用户名字符合法性验证
            # 密码长度验证
            # 更多的其它验证.....
            try:
...
"""
主页面的书籍前端显示
Book.objects.all().filter().order_by('id')查询所有符合条件的按照id排序的
"""
#@login_in
def all_book(request):
    page_number = request.GET.get("page", 1)    # 总页数
    books = Book.objects.all().filter().order_by('id')  # 从数据库里调用书籍，按照字段名来选择排序
    paginator = Paginator(books, 12)
...
def search(request):  # 搜索
    ...
    books = Book.objects.filter(
        Q(title__icontains=key) | Q(author__icontains=key)
    )  #对书名标题进行搜索
...
'''
详细信息界面内容,book.html
request.session.get获取存储在Session的数据
'''
#@login_in
def book(request, book_id):
    ...
    book.num += 1
    book.save()
    comments = book.comment_set.order_by("-create_time")    # 按最新时间给评论排序
...
def hot_book(request):
    ...
    books=Book.objects.all().filter().order_by('-book_pl')
...
def highscore(request):
    page_number = request.GET.get("page", 1)
    books = Book.objects.all().filter().order_by('-book_star')
    books = books_paginator(books[:100], page_number)
    return render(request, "user/item.html", {"books": books, "title": "好评书籍"})
